...
The capstone under analysis is a threat-aware quantum-routing framework augmented by the governed fairness layer developed in DSCI601. At each decision round, the routing pipeline observes a possibly incomplete network state, uses a bandit policy to select a path or routing action, and allocates limited qubit or entanglement-generation capacity under stochastic link behavior and configurable threats. The existing implementation includes fairness run and outcome contracts, a quantum fairness adapter, group-aware evaluation, and the EQUITAS-Quantum mediator. Those components have been exercised through unit and old-pipeline smoke tests, but full-scale quantum fairness experiments have not yet been completed. Quantum networking makes the underlying decisions consequential because entanglement distribution depends on probabilistic link success, limited resources, and routing choices across competing requests~\cite{wehner2018quantum,pant2019routing}.

Fairness in this setting is allocative: a controller can achieve strong network-wide efficiency while some flows receive systematically worse service. The existing fairness contract can normalize a prespecified group together with success, latency, reward, and relevant execution context. This analysis extends that contract by requiring declared demand, allocation, retry count, context quality, and starvation age wherever the legacy quantum trace does not yet expose them. The initial equity units are operational service classes or flow cohorts fixed before evaluation, such as latency-sensitive versus delay-tolerant traffic, high- versus low-demand requests, or high- versus low-context-quality cohorts. They are not assumed to be demographic groups and must not be defined after observing outcomes. The affected stakeholders include applications or tenants competing for service, network operators accountable for allocation policy, and downstream users who depend on the resulting entanglement paths. If a future deployment associates service classes with people or institutions, protected-attribute analysis and stakeholder review would also be required.

This scope matters because a network-wide efficiency score can conceal persistent differences in access, delay, retries, or successful entanglement. Routing and resource allocation therefore remain the primary technical problem, but aggregate performance alone is not an adequate fairness evaluation. Quantum-network routing and fair-sharing research likewise treats scarce capacity and simultaneous requests as allocation problems rather than throughput questions alone~\cite{li2021effective,cicconetti2023service}. The fairness analysis asks whether the controller distributes useful quantum service equitably across prespecified operational groups while respecting the physical and resource constraints of the network.
